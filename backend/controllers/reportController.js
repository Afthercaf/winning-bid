...
import { uploadImageToCloudinary } from "../imgurService.js"; // Asegúrate de usar la ruta correcta
...
export const createReport = [
  ...
  async (req, res) => {
    try {
      const { productId, description, category } = req.body;
      ...
      const reportedId = product.seller_id._id;

      // Subir imágenes del reporte a Cloudinary
      const images = await Promise.all(
        req.files.map(async (file) => {
          try {
            ...
            console.error(
              `Error al subir la imagen ${file.originalname}:`,
              error
            );
            return null;
          }
      ...
      const newReport = new Report({
        ...
        description,
        category,
        seller: reportedId, // Añadir el seller_id del producto
      });
...
export const getReports = async (req, res) => {
  try {
    const reports = await Report.find()
      ...
      .populate("seller", "name email"); // Añadir el seller a la población
...
export const updateReportStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al actualizar el estado del reporte", error });
  }
};
...
export const getReportsByUserId = async (req, res) => {
  try {
    ...
    console.error(
      "Error al obtener los reportes por ID de usuario:",
      error.message
    );
    res.status(500).json({ error: "Error al obtener los reportes" });
  }
...
export const getReportsForReportedUser = async (req, res) => {
  try {
    ...
    const reports = await Report.find({ reported: userId })
      ...
      .populate("seller") // Añadir el seller a la población
      .lean();

    if (!reports.length) {
      return res
        .status(404)
        .json({ message: "No hay reportes para este usuario" });
    }

    res.status(200).json(reports);
  } catch (error) {
    console.error(
      "Error al obtener reportes para el usuario reportado:",
      error.message
    );
